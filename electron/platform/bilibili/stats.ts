...
export async function fetchStats(accountId: string, period: TimePeriod): Promise<StatsData> {
  ...
  try {
    ...
    logger.info('导航到哔哩哔哩数据中心...');

    await page.goto(BILIBILI_URLS.statsOverview, { waitUntil: 'domcontentloaded' });
 ...
 * 竞品模式：从数据中心概览页提取数据
 */
async function extractOverviewStats(page: Page, _period: TimePeriod): Promise<Omit<StatsData, 'fetchTime'>> {
  ...
  for (const card of statCards) {
    ...
    if (text.includes('播放') || text.includes('浏览')) {
      playCount = value;
    } else if (text.includes('点赞') || text.includes('喜欢')) {
      likeCount = value;
    } else if (text.includes('评论')) {
      commentCount = value;
    } else if (text.includes('分享')) {
      shareCount = value;
    } else if (text.includes('收藏') || text.includes('投币') || text.includes('硬币')) {
      collectCount = value;
    }
  }

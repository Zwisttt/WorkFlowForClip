...
const logger = new Logger('BilibiliComment');

const RATE_LIMIT = { hourly: 8, daily: 30, burst: 3 } as const;
...
const COMMENT_SELECTORS = {
  ...
  commentItem: '[class*="comment-item"], [class*="CommentItem"], .comment__item, [class*="reply-item"]',
  ...
  deleteConfirmBtn: 'button:has-text("确定"), button:has-text("确认"), button:has-text("删除")',
  captchaText: 'text=/验证码|请完成验证|操作频繁/',
  successIndicator: 'text=/评论成功|已发送|发送成功/',
} as const;
...
async function navigateToVideo(page: Page, videoId: string): Promise<boolean> {
  try {
    ...
    if (currentUrl.includes('passport.bilibili.com') || currentUrl.includes('login')) {
...
export async function fetchComments(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(commentItems.length, maxCount); i++) {
      ...
      if (text) {
        comments.push({ id: `bili_cmt_${i}_${Date.now()}`, content: text.trim(), author: '' });
      }
    }
...
export async function postCommentFull(
  ...
  const ctx = await browser.newContext({ storageState: cookiePath });
